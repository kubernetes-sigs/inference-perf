...
Four coordination mechanisms handle this:
...
- **`shared_failed_sessions`** — worker→main failure notification. An `mp.Manager().list()`
  that allows workers to notify the main process when a session fails. When `process_failure()`
  is called, the worker adds the session_id to this shared list. The main process checks this
  list when ending OTEL session spans to mark failed sessions with error messages.

Each `OTelChatCompletionAPIData` holds references to `registry`, `worker_tracker`,
`completion_queue`, and `shared_failed_sessions`. Before dispatching an HTTP request, the worker
calls `wait_for_predecessors_and_substitute()`, which awaits predecessors via
`registry.require_async()` (zero threads — pure `asyncio.Event` suspension), checks
`worker_tracker` for session failure before and after waiting, then substitutes output segments
with actual predecessor text. After the response returns, `on_completion()` writes to `registry`
(unblocking dependents) and `worker_tracker` (recording completion), and pushes to
`completion_queue` if this was the last node in the session. If a request fails,
`process_failure()` marks the session as failed in both `worker_tracker` and
`shared_failed_sessions`.
...
- `shared_failed_sessions` (an `mp.Manager().list()`) allows workers to notify the main process of failures
- When a worker marks a session as failed, it also adds the session_id to this shared list
- The main process (load generator) checks this list when ending OTEL session spans
- Failed sessions are marked with an error message in their OTEL span: `"Session failed"`
