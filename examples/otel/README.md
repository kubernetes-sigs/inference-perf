...
Two orthogonal coordination objects handle this:
...
- **`SessionSharedState`** — cross-process. Holds two `Manager.dict()` instances: one mapping
  `node_id → completion_time` (so the main process can detect session completion) and one
  mapping `session_id → failed` (so workers can skip dependents after a failure). This is the
  only place in the OTel replay path where multiprocessing proxies are created.

Each `OTelChatCompletionAPIData` holds references to both. Before dispatching an HTTP request,
the worker calls `wait_for_predecessors_and_substitute()`, which awaits predecessors via
`registry.require_async()` (zero threads — pure `asyncio.Event` suspension), checks
`shared_state` for session failure before and after waiting, then substitutes output segments
with actual predecessor text. After the response returns, `on_completion()` writes to both
`registry` (unblocking dependents on the same worker) and `shared_state` (signalling the main
