...
The price–performance metrics below are not directly reported by Inference Perf but can be computed from the metrics reported by the tool using the cost of the underlying hardware. @inference-perf-metrics provides the formulas to compute these additional metrics.
