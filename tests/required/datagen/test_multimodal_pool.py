...
from inference_perf.payloads import SyntheticAudioSpec, SyntheticImageSpec
...
async def test_payload_audio_pool_bounds_distinct_durations() -> None:
    ...
    for aud in item.multimodal_spec.audios:
        assert isinstance(aud, SyntheticAudioSpec)
...
async def test_shared_prefix_pool_does_not_affect_prefix_bytes() -> None:
    ...
    for _ in range(6):
        ...
        payload = await api_data.to_request_body(effective_model_name="m", max_tokens=10, ignore_eos=False, streaming=False)
