...
class TestPreprocessSaturation(unittest.IsolatedAsyncioTestCase):
    async def test_preprocess_generates_1_75x_saturation(self) -> None:
        # Setup
        mock_datagen = MagicMock(spec=RandomDataGenerator)
        ...
        rates = [
            ...
            if isinstance(stage, (StandardLoadStage, ConcurrentLoadStage)) and stage.rate is not None
        ]
        max_rate = max(rates) if rates else 0.0

        print(
            f"Generated rates: {[s.rate for s in generator.stages if isinstance(s, (StandardLoadStage, ConcurrentLoadStage))]}"
        )
