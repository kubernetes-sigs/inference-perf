...
class TestMultiTurnHang(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_worker_simulation_no_hang(self) -> None:
        ...
        await data_0.to_request_body("model", 10, True, False)
        ...
        try:
            await asyncio.wait_for(data_1.to_request_body("model", 10, True, False), timeout=2.0)
    ...
    async def test_update_context_idempotent_on_double_release(self) -> None:
        ...
        await session.get_context(0)  # acquire (to_request_body)
