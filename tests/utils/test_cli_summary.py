...
class TestCliSummary(unittest.TestCase):
    ...
    def test_print_summary_table_empty(self, mock_rprint):
        print_summary_table([])
        mock_rprint.assert_called_once_with(
            "[yellow]No per-stage lifecycle metrics found to display summary table.[/yellow]"
        )

    @patch('inference_perf.utils.cli_summary.Console.print')
    def test_print_summary_table_with_reports(self, mock_console_print):
