...
from inference_perf.config import DistributionConfig, DistributionType
...
class TestSampleFromDistribution:
    def test_normal_within_bounds(self) -> None:
        config = DistributionConfig(distribution=DistributionType.NORMAL, mean=500.0, min=100, max=900, std_dev=100.0)
    ...
    def test_normal_approximate_mean(self) -> None:
        config = DistributionConfig(distribution=DistributionType.NORMAL, mean=200.0, min=10, max=2000, std_dev=50.0)
    ...
    def test_skew_normal_within_bounds(self) -> None:
        config = DistributionConfig(
            distribution=DistributionType.SKEW_NORMAL, mean=200.0, min=10, max=2000, std_dev=80.0, skew=2.5
        )
        result = sample_from_distribution(config, 1000, rng=np.random.default_rng(42))
    ...
    def test_skew_normal_positive_skew_has_right_tail(self) -> None:
        config = DistributionConfig(
            distribution=DistributionType.SKEW_NORMAL, mean=100.0, min=0, max=1000, std_dev=50.0, skew=5.0
        )
        result = sample_from_distribution(config, 10000, rng=np.random.default_rng(42))
    ...
    def test_skew_normal_zero_skew_approximates_normal(self) -> None:
        config_skew = DistributionConfig(
            distribution=DistributionType.SKEW_NORMAL, mean=500.0, min=0, max=1000, std_dev=100.0, skew=0.0
        )
        config_normal = DistributionConfig(distribution=DistributionType.NORMAL, mean=500.0, min=0, max=1000, std_dev=100.0)
    ...
    def test_lognormal_within_bounds(self) -> None:
        config = DistributionConfig(distribution=DistributionType.LOGNORMAL, mean=150.0, min=1, max=4096, std_dev=60.0)
        result = sample_from_distribution(config, 1000, rng=np.random.default_rng(42))
        assert result.min() >= 1
        assert result.max() <= 4096

    def test_lognormal_zero_std_dev_constant(self) -> None:
        config = DistributionConfig(distribution=DistributionType.LOGNORMAL, mean=100.0, min=1, max=500, std_dev=0.0)
    ...
    def test_lognormal_negative_mean_raises(self) -> None:
        config = DistributionConfig(distribution=DistributionType.LOGNORMAL, mean=-5.0, min=-10, max=100, std_dev=10.0)
        with pytest.raises(ValueError, match="mean > 0"):
            sample_from_distribution(config, 100)

    def test_uniform_within_bounds(self) -> None:
        config = DistributionConfig(distribution=DistributionType.UNIFORM, mean=50.0, min=10, max=100)
        result = sample_from_distribution(config, 1000, rng=np.random.default_rng(42))
        assert result.min() >= 10
        assert result.max() <= 100

    def test_uniform_covers_range(self) -> None:
        config = DistributionConfig(distribution=DistributionType.UNIFORM, mean=50.0, min=1, max=10)
        result = sample_from_distribution(config, 10000, rng=np.random.default_rng(42))
    ...
    def test_poisson_within_bounds(self) -> None:
        config = DistributionConfig(distribution=DistributionType.POISSON, mean=4.2, min=0, max=50)
        result = sample_from_distribution(config, 1000, rng=np.random.default_rng(42))
        assert result.min() >= 0
        assert result.max() <= 50

    def test_poisson_approximate_mean(self) -> None:
        config = DistributionConfig(distribution=DistributionType.POISSON, mean=10.0, min=0, max=100)
        result = sample_from_distribution(config, 10000, rng=np.random.default_rng(42))
    ...
    def test_fixed_value_std_dev_zero(self) -> None:
        config = DistributionConfig(distribution=DistributionType.NORMAL, mean=50.0, min=50, max=50, std_dev=0.0)
        result = sample_from_distribution(config, 100, rng=np.random.default_rng(42))
        assert all(v == 50 for v in result)

    def test_invalid_count_zero(self) -> None:
        config = DistributionConfig(distribution=DistributionType.NORMAL, mean=50.0, min=10, max=100, std_dev=10.0)
    ...
    def test_invalid_min_greater_than_max(self) -> None:
        ...
        with pytest.raises(Exception, match="min.*max"):
            DistributionConfig(distribution=DistributionType.NORMAL, mean=50.0, min=100, max=10, std_dev=10.0)
