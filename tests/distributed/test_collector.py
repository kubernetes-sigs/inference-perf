...
@pytest.mark.asyncio
async def test_reload_metrics(mock_redis_client):
...
@pytest.mark.asyncio
async def test_reload_metrics_not_connected(mock_redis_client):
...
@pytest.mark.asyncio
async def test_reload_metrics_exception_handling(mock_redis_client):
    collector = RedisRequestDataCollector(mock_redis_client, "results_stream")
    
    # Return invalid JSON
    mock_redis_client.redis.xread.side_effect = [
        [("results_stream", [("1-0", {b"data": "invalid json"})])],
        []
    ]
...
@pytest.mark.asyncio
async def test_start(mock_redis_client):
    collector = RedisRequestDataCollector(mock_redis_client, "results_stream")
