...
from inference_perf.config import Config, LoadConfig, DataConfig, APIConfig, StandardLoadStage
...
def mock_configs():
    load_config = LoadConfig(
        stages=[StandardLoadStage(rate=10, duration=2)]
    )
...
async def test_run_job_with_configs(mock_datagen_class, mock_tokenizer_class, mock_redis_client, mock_configs):
    ...
    orchestrator.wait_for_completion = AsyncMock()
...
@pytest.mark.asyncio
async def test_wait_for_completion(mock_redis_client):
    orchestrator = Orchestrator(mock_redis_client)
...
@pytest.mark.asyncio
async def test_wait_for_completion_cancellation(mock_redis_client):
    orchestrator = Orchestrator(mock_redis_client)
...
async def test_start_daemon(mock_config_class, mock_redis_client):
    orchestrator = Orchestrator(mock_redis_client)
    ...
    orchestrator.run_job_with_configs = AsyncMock()
