...
@pytest.mark.asyncio
async def test_redis_event_output_registry_record() -> None:
    ...
    mock_redis_client.record_event_output.assert_called_once_with(
        "test_job", "event_1", "output_text", [{"role": "user", "content": "hi"}]
    )
...
@pytest.mark.asyncio
async def test_redis_event_output_registry_require_async_hit() -> None:
    mock_redis_client = MagicMock(spec=RedisClient)
...
@pytest.mark.asyncio
async def test_redis_worker_session_tracker() -> None:
    mock_redis_client = MagicMock(spec=RedisClient)
