...
@pytest.mark.asyncio
async def test_create_consumer_group_success(mock_redis):
    client = RedisClient()
    await client.connect()
...
@pytest.mark.asyncio
async def test_create_consumer_group_busygroup(mock_redis):
    client = RedisClient()
    await client.connect()
...
@pytest.mark.asyncio
async def test_create_consumer_group_other_error(mock_redis):
    client = RedisClient()
    await client.connect()
...
@pytest.mark.asyncio
async def test_read_tasks(mock_redis):
    client = RedisClient()
    await client.connect()
    mock_redis.xreadgroup.return_value = [
        ("mystream", [("123-0", {"data": json.dumps({"key": "value"})})])
    ]
...
@pytest.mark.asyncio
async def test_ack_task(mock_redis):
    client = RedisClient()
    await client.connect()
...
@pytest.mark.asyncio
async def test_publish_telemetry(mock_redis):
    client = RedisClient()
    await client.connect()
...
@pytest.mark.asyncio
async def test_add_result(mock_redis):
    client = RedisClient()
    await client.connect()
...
@pytest.mark.asyncio
async def test_get_subscriber(mock_redis):
    client = RedisClient()
    await client.connect()
