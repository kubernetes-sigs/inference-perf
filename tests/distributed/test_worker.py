...
@pytest.fixture
def mock_model_server_client():
    client = MagicMock(spec=ModelServerClient)
    client.process_request = AsyncMock()
    return client

@pytest.mark.asyncio
async def test_worker_run(mock_redis_client, mock_model_server_client):
...
@pytest.mark.asyncio
async def test_process_task(mock_redis_client, mock_model_server_client):
    worker = DistributedWorker(
        worker_id="w1",
        redis_client=mock_redis_client,
        client=mock_model_server_client,
    ...
    worker.collector.get_metrics = MagicMock(return_value=[mock_metric])
