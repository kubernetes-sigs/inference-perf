...
def _mock_metric(
    *,
    ...
    m.info.input_tokens = input_tokens
    m.info.response_info = StreamedInferenceResponseInfo(
        response_chunks=[],
...
def test_lifecycle_report_shape_with_failures() -> None:
    ...
    failure.info.payload = Payload(text=Text(input_tokens=80, output_tokens=0))
    failure.info.input_tokens = 80
    failure.info.response_info = None
