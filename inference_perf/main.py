...
def main_cli() -> None:
    ...
    if config.data:
        ...
        if config.data.type == DataGenType.ShareGPT:
            datagen = HFShareGPTDataGenerator(config.vllm.api, None, vllm_client.tokenizer)
            ...
            datagen = SyntheticDataGenerator(
                apiType=config.vllm.api, ioDistribution=io_distribution, tokenizer=vllm_client.tokenizer
            )
        else:
