...
class GMPMetricsCollector(PrometheusMetricsCollector):
    def __init__(self, metrics: List[PrometheusMetric], config: GMPCollectorConfig) -> None:
    ...
    async def query_metric(self, query: str, duration: float) -> Optional[float]:
        ...
        request_post = requests.get(url=self.url, headers=headers_api, params={"query": query})
