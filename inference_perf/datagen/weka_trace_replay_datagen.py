...
"""Weka (Agent-X) Trace Replay Data Generator.

This module provides a data generator that replays Weka KV-cache-tester traces
(e.g., from Hugging Face semianalysisai/cc-traces-weka-*). It deterministically
reconstructs the prompt and assistant response text of each turn from the
trace's hash IDs, then builds a dependency graph and replays them using the
...
WekaRequest = Annotated[
    Union[WekaNormalRequest, WekaStreamingRequest, WekaSubagentEntry],
...
class HashIdRandomGenerator:
    ...
    def reseed_for_hash_id(self, hash_id: int) -> None:
        seed_bytes = hashlib.sha256(
            f"{self.base_seed}:{self._trace_id}:{hash_id}".encode()
        ).digest()
    ...
    def _reconstruct_raw_calls(self, trace: WekaTrace) -> List[RawCall]:
        ...
        for k, (outer_idx, req) in enumerate(parent_plan.normals):
            ...
            if k + 1 < len(parent_plan.normals):
                ...
                lookahead._segments = [RoleSegment(s.role, s.block_start, s.block_count, list(s.tokens), s.content) for s in parent_recon._segments]
        ...
        for cp in child_plans:
            ...
            for k, creq in enumerate(cp.stream_requests):
                ...
                if k + 1 < len(cp.stream_requests):
                    ...
                    lookahead._segments = [RoleSegment(s.role, s.block_start, s.block_count, list(s.tokens), s.content) for s in child_recon._segments]
