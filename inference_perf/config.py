...
class MediaPoolConfig(BaseModel):
    ...
    size: int = Field(
        description=(
            "Total number of distinct media items materialized once at pool init "
            "and reused across requests. Must be >= 1."
        ),
        ge=1,
    )
...
class MediaDatagenConfig(BaseModel):
    ...
    pool: Optional[MediaPoolConfig] = Field(
        default=None,
        description=(
            "Optional bounded pool of distinct pre-rendered items for this modality. "
            "See ``MediaPoolConfig``."
        ),
    )
