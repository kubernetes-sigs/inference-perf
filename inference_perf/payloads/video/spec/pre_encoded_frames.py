...
class PreEncodedFramesVideoSpec(VideoSpec):
    ...
    def get_metrics(self, wire_bytes: int) -> Video:
        # Bytes are the sum of caller-supplied frame blobs. Geometry comes
        # from the spec's declared ``width``/``height`` — the loader is
        # responsible for populating those (typically from PyAV stream info),
        # and they're trusted as authoritative.
        return Video(
