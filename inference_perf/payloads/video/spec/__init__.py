...
- :class:`RemoteVideoSpec` (``remote.py``) — stub for URL-referenced video
  fetched server-side; in :data:`VideoSpecUnion` but not yet materializer-wired.
- :class:`LocalFileVideoSpec` (``local.py``) — stub for an on-disk video file
  read at materialization time (single-file / MP4 case); in
  :data:`VideoSpecUnion` but not yet materializer-wired.

Use :data:`VideoSpecUnion` when typing a list/field that holds any concrete
subclass — Pydantic discriminates on the ``kind`` literal. Constructing a
stub variant succeeds; passing one through the materializer raises
``TypeError`` until that variant's branch lands.
"""
