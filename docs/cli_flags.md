...
| `--load.request_retries` | int | How many extra attempts to make when a request fails before it ever produced a response, i.e. a connection was refused, reset, or dropped from the pool. Only such pre-first-byte transport faults are retried: once any response byte has arrived the request has already produced a TTFT, so retrying it would report the retry's latency instead of the original's. Timeouts are never retried. Defaults to 0, which preserves the historical behavior of failing on the first fault. Note that request_timeout applies per attempt, so the worst-case wall time for one request becomes (1 + request_retries) * request_timeout plus backoff. |
