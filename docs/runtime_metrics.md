...
| `inference_perf_stage_requests_planned` | Gauge | `stage` | Always | Requests the stage set out to issue. Absent for stages whose request count is not known up front. |
| `inference_perf_stage_requests_finished` | Gauge | `stage` | Always | Requests the stage is done with, whatever became of them, including any abandoned before dispatch. Read against inference_perf_stage_requests_planned for stage completion; this is the same count the stage's own termination check reads. |
| `inference_perf_stage_requests_skipped` | Gauge | `stage` | Always | Requests counted as finished that were never sent, because the session had already failed or the request could not be built. These produce no lifecycle metric, so they appear in no outcome counter and in no report. |
| `inference_perf_stage_sessions_planned` | Gauge | `stage` | Always | Sessions the stage set out to run. Only present for session-based stages. |
| `inference_perf_stage_sessions_finished` | Gauge | `stage` | Always | Sessions the stage is done with, including any skipped because their graph could not be built. Read against inference_perf_stage_sessions_planned for stage completion. |
| `inference_perf_stage_sessions_skipped` | Gauge | `stage` | Always | Sessions counted as finished whose graph could not be built, so they dispatched nothing and produced no session lifecycle metric. |
| `inference_perf_stages_completed` | Gauge | none | Always | Stages that have ended, whether they completed or were cut short. Read against inference_perf_stages. |
